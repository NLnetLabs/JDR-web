  ...
  <div>
    <el-card class="box-card">
      <div class="text item">
        ...
        <panZoom :options="{ minZoom: 0.2, maxZoom: 2, bounds: true, boundPadding: 0 }">
          <div>
  ...
  methods: {
    getTreeData: function(inputTree) {
      ...
      function traverse(node) {
        node.image_url = LIGHT_GREEN_SQUARE;
        if (node.name === "root") {
          node.image_url = GREEN_SQUARE;
        }
        if (node.name === "rpki.ripe.net") {
          node.image_url = RIPENCC;
        }
        if (node.name === "repository.lacnic.net") {
        ...
        if (children && children.length) {
          children.forEach(child => {
            child.image_url = LIGHT_GREEN_SQUARE;
